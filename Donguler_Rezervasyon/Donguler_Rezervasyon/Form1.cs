...
namespace Donguler_Rezervasyon
{
    public partial class Form1 : Form
    {
        ...
        {
            if (cbKoltukTipi.SelectedItem == "Pencere")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
                    {
                        ...
                        //Turnary if
                        // 1. Kullanım
                        btn.Enabled = koltuk.xKonumu == 0 || koltuk.xKonumu == 3; // Pencere kenarıdır
                        
                        // 2. Kullanım
                        btn.Enabled = (koltuk.xKonumu == 0 || koltuk.xKonumu == 3) ? true : false;
                        btn.AccessibleDescription = (koltuk.xKonumu == 0 || koltuk.xKonumu == 3) ? "Pencere Kenarı" : "Koridor";
                        
                        // 3. Kullanım
                        if (koltuk.xKonumu == 1 || koltuk.xKonumu == 2) // Koridor koltuğu
                        {
                            btn.Enabled = false;
                        }
                        else //Pencere
            ...
            else if (cbKoltukTipi.SelectedItem == "Koridor")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
                    {
                        Button btn = (control as Button);
                        Koltuk koltuk = ((Koltuk)btn.Tag);
                        if (koltuk.xKonumu == 0 || koltuk.xKonumu == 3) // Pencere koltuğu
                        {
                            btn.Enabled = false;
                        }
                        else
        ...
        {
            if (lbKoltukTipi.SelectedItem == "Hiçbiri")
            {
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
            ...
            else if (lbKoltukTipi.SelectedItem == "Erkek")
            {
                ...
                foreach (Control control in scAnaPanel.Panel2.Controls)
                {
                    if (control is Button)
                    {
                        Button btn = (control as Button);
                        Koltuk koltuk = ((Koltuk)btn.Tag);
                        // Yan koltuğu erken olanların enable yapacağız
                        Koltuk yanKoltuk = YanKoltuguGetir(koltuk);
                        if(yanKoltuk.cinsiyetTipi == CinsiyetTipi.Erkek)
                            btn.Enabled = true;
                        else
            ...
            else if (lbKoltukTipi.SelectedItem == "Kadın")
            {
